...
ShopReference %<>% dplyr::mutate(city = as.factor(city),shop_id = as.integer(shop_id))
...
ArrangeData <- function(data = MonthlySales.ItemShop, CategorySuperlative = NULL, item_category_id = NULL, CategoryDesc = NULL, ...){
  opts <- list(...) # How can we use High level Category filters?
  
  # filter by super category
  if(!is.null(CategorySuperlative)){
   data %<>% dplyr::filter_at(vars(which(colnames(.) %in% CategorySuperlative)),all_vars(as.integer(.)== 2))
  }
