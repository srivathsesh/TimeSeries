...
```{r setup, include=F,warning=FALSE,message=F,error=FALSE,fig.show='asis'}
...
```{r, fig.cap="Monthly number of people on unemployment benefits in Australia",fig.height=4,fig.pos="h",out.extra=''}
...
```{r,fig.height=6,fig.cap = "Top: Total accidental deaths by month in the US, Middle: Monthly Average Accidental deaths per day, Bottom: Transformed monthly average accidental deaths per day ",fig.pos="h",out.extra=''}
data("usdeaths")
par(mfrow = c(3,1))
...
```{r, fig.cap= "Top: Quarterly production of bricks (in millions) at Portland, Australia, Bottom : BoxCox transformed Quarterly production of bricks",out.extra='',fig.height=8}
data("bricksq")
par(mfrow = c(2,1))
...
```{r, fig.cap="Time series modeling of Dow Jones industrial average",fig.pos="h",out.extra=''}
data("dowjones")
...
```{r, fig.cap="Autocorrelation plot of residuals",fig.pos="h",out.extra=''}
...
```{r,fig.cap="Residual Analysis",fig.pos="h",out.extra=''}
par(mfrow = c(2,2))
...
```{r, fig.cap="IBM Stock price at close",fig.pos="h",out.extra=''}
data("ibmclose")
...
```{r,fig.cap="Residual Analysis",fig.pos="h",out.extra='',fig.height=4}
...
```{r,fig.cap="IBM stock price forecast",fig.pos="h",out.extra='',fig.height=4}
...
```{r, fig.cap = "Time series plot of single family home sales in the US",fig.pos="h",out.extra='',fig.show="as.is"}
data("hsales")
par(mfrow = c(2,2))
plot(hsales)
...
```{r, fig.cap = "Electricity consumption by temperature",fig.pos="h",out.extra='',fig.height=3,fig.show="asis"}
library(ggplot2)
...
```{r,fig.cap="Residual plots",fig.pos="h",out.extra='',fig.height=4}
...
```{r,fig.cap="Winning times of 400 meters dash",fig.pos="h",out.extra=''}
data("olympic")
...
```{r,fig.cap="Time series plot of sales "}
data("plastics")
plot(plastics)
```
...
```{r, fig.cap = "Classical multiplicative decomposing"}
DecomposeMulti <- decompose(plastics, type = "multiplicative")
plot(DecomposeMulti)
```


d,g and h ) Figure 16 shows the time series plot of the seasonally adjusted data. 
...
f) Figure 18 shows the decomposition when 500 units is added to Nov of year 5. The effect is none, this is because of the classical decomposition that has a disadvantage of losing predictions in the begining and ending of the time series (loss of half year each).
Figure 19 Shows the decomposition when 500 units is added to June of year 4. This has a slight effect on the predictions and it can be seen in the random error chart as a spike. The moving average smoothes over the spike to minimize its effect. 

```{r,fig.cap = "Added 500 to december of year 5"}
plastics.rev <- plastics
plastics.rev[59] <- plastics.rev[59] + 500
plot(decompose(plastics.rev, type = "multiplicative"))
```

```{r,fig.cap = "Added 500 to Jun of Year 4"}
