...
sales_train %>% dplyr::select(item_id,item_cnt_day) %>% dplyr::group_by(item_id) %>% skimr::skim() -> summarytrain

summarytrain %<>% dplyr::filter(stat %in% c('mean','p50', 'p0','p75','p100')) %>% 
  select(item_id,stat,formatted) %>% tidyr::spread(stat,formatted) %>% 
  select(item_id,p0,p50,mean,p75,p100) %>% mutate_if(is.character,as.numeric) %>% 
  mutate(High = p100 - p50, Low = p50-p0) 
  summarytrain %>% ggplot(aes(x = item_id, y = High)) + geom_point() ->  p
  ggplotly(p)
  summarytrain %>% ggplot(aes(x = item_id, y = Low)) + geom_point() -> q
  ggplotly(q)
  
Highoutlieritems <- summarytrain[which(summarytrain$High > 100),"item_id"]

sales_train %>% dplyr::filter(item_id %in% Highoutlieritems$item_id) %>%
  dplyr::group_by(item_id,shop_id) %>% 
  summarise(Max = max(item_cnt_day),Median = median(item_cnt_day)) %>% dplyr::ungroup() %>% 
  dplyr::group_by(item_id) -> OutlierInfo
 OutlierInfo %>%  dplyr::summarise(MaxMax = max(Max)) -> OutlierInfoSel

sales_train %<>% mutate(impute = ifelse(item_id %in% !!OutlierInfoSel$item_id & item_cnt_day %in% OutlierInfoSel$MaxMax,1,0))

# impute values
implookup <- dplyr::inner_join(OutlierInfo, OutlierInfoSel,by = c('item_id' = 'item_id','Max' = 'MaxMax'))
sales_train %<>% dplyr::mutate(ImputedVal = NA)
colnames(implookup)[4] <- 'ImputedVal'

sales_train_imp <- merge(sales_train,implookup[,-3],by = c("item_id"="item_id","shop_id"="shop_id")) %>% dplyr::mutate(impitem_cnt_day = ifelse(impute==1,ImputedVal.y,item_cnt_day))
...
#                               Aggregated data
#-------------------------------------------------------------------------------------------------- 
Agg_sales_train <- sales_train_imp %>% 
                 ...
                 dplyr::summarise(MonthlyCts = sum(impitem_cnt_day),Price = mean(item_price,na.rm = T)) %>% 
  ...
  if(all(is.element(c("shop_ids","item_ids"),ResponseCriteria))){
    ...
    if(all(is.element(c("shop_ids","category_id"),ResponseCriteria))){
      ...
      if(all(is.element(c("shop_ids","CategorySuper"),ResponseCriteria))){
        ...
        if(all(is.element(c("category_id"),ResponseCriteria))){
      Response <- data  %>% 
        dplyr::filter(item_category_id == !!response$category_id) %>% 
        dplyr::select(date_block_num,item_category_id,MonthlyCts,Price) %>% 
        dplyr::arrange(date_block_num,item_category_id) %>% 
        dplyr::group_by(date_block_num,item_category_id) %>% 
        dplyr::summarise(MonthlyCts = sum(MonthlyCts, na.rm = T), Price = mean(Price,na.rm = T)) %>% 
        dplyr::mutate(itmCatid.ShopID = as.character(item_category_id + shop_id/100))
...
```{r}
MonthlySales.ItemShop %>% dplyr::select(item_category_id, MonthlyCts) %>% dplyr::group_by(item_category_id) %>% skimr::skim() -> SummaryStats

SummaryStats %>% dplyr::filter(stat %in% c('mean','p50', 'p0','p75','p100')) %>% 
  select(item_category_id,stat,formatted) %>% tidyr::spread(stat,formatted) %>% 
  select(item_category_id,p0,p50,mean,p75,p100) %>% mutate_if(is.character,as.numeric) %>% 
  mutate(High = p100 - p50, Low = p50-p0)
...
```{r}
MonthlySales.ItemShop %>% dplyr::group_by(date_block_num,Category,City) %>% dplyr::summarise(Sales = sum(MonthlyCts)
```
